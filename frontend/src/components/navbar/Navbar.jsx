            ...
            {!isLandingPage && (
                <nav className="flex justify-between items-center px-3 py-7 shadow-lg fixed top-0 w-full sm:flex-col sm:justify-start sm:items-start sm:w-64 sm:fixed sm:h-screen z-10 sm:top-0 bg-white">
                    ...
                    <ul className="hidden sm:block">
                        ...
                        <li className="p-4 flex justify-center items-center w-56 text-lg font-mono absolute bottom-2 cursor-pointer bg-slate-400 rounded-lg hover:scale-105 transition-transform hover:shadow-lg text-white" onClick={()=>{navigate('/'); setIsMobile(!isMobile)}}>
                            logout
                        </li>
                    </ul>
                    <ul
