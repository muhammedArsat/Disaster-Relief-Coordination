...
import{postReliefForm} from'../../apis/Api';
...
const PostForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const nameParts = selectedDistrict.name;
    const [districtName] = nameParts.split(" lat:");
    const latMatch = nameParts.match(/lat:\s*(-?\d+(\.\d+)?)/);
    const lngMatch = nameParts.match(/lng:\s*(-?\d+(\.\d+)?)/);
    
    const latitude = latMatch ? parseFloat(latMatch[1]) : null;
    const longitude = lngMatch ? parseFloat(lngMatch[1]) : null;
  
    const data = {
      disaster: e.target.disaster.value,
      location: {
      ...
      childCount: Number(e.target.childCount.value),
      femaleCount: Number(e.target.femaleCount.value),
      maleCount: Number(e.target.maleCount.value),
      food: Number(e.target.food.value),
      sanitization: Number(e.target.sanitization.value),
      cloth: Number(e.target.cloth.value),
      medical: Number(e.target.medical.value),
    };
  
    try {
      const response = await postReliefForm(data);
